#!/usr/bin/env bun
// Fail a pull request against swxtch that does not add its entry to CHANGESET.md.
//
// CHANGESET.md records what this fork carries on top of upstream. It only stays true if
// the PR making a change records it, so CI checks for a `- **#<number>**` entry. Exempt:
// Dependabot, and sync-upstream-<timestamp> branches, whose entry sync-upstream.ts writes.
//
...
export function checkChangeset(doc: string | undefined, pr: PullRequest) {
  ...
  // Exactly the names sync-upstream.ts generates, so a hand branch cannot opt out by prefix.
  if (/^sync-upstream-\d{8}-\d{6}$/.test(pr.head ?? ""))
    return { ok: true, reason: `#${number} is an upstream sync, recorded by sync-upstream.ts` }
